...
export class BatteryService {
  ...
  async init() {
    ...
    // Sự kiện đổi trạng thái (cắm sạc / rút sạc / đầy pin)
    Battery.addBatteryStateListener(async ({ batteryState }) => {
      const level = await Battery.getBatteryLevelAsync();
      await this.handleUpdate(level, batteryState);
    });

    // Sự kiện đổi % pin — bắt cả lúc pin tụt dần khi KHÔNG cắm/rút sạc,
    // trước đây không có listener này nên giữa 2 lần cắm/rút sạc, app không
    // ghi nhận % pin thay đổi, dữ liệu biểu đồ/báo cáo bị sai lệch.
  ...
  private async handleUpdate(level: number, state: Battery.BatteryState) {
    ...
    // Guard bằng this.session để không tạo phiên sạc mới mỗi lần % pin nhích
    // lên trong lúc đang sạc (addBatteryLevelListener bắn liên tục).
